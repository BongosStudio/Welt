...
namespace Welt.Core
{
    public class QueryProtocol
    {
        ...
        public void Start()
        {
            Port = Server.ServerConfiguration.QueryPort;
        ...
        private void HandleHandshake(byte[] buffer, IPEndPoint clientEP)
        {
            using (var ms = new MemoryStream(buffer))
            {
                using (var stream = new BinaryReader(ms))
                {
                    ...
                    {
                        while (!UserList.TryRemove(clientEP, out var u))
                            Thread.Sleep(1);
                    }
        ...
        private Dictionary<string, string> GetStats()
        {
            ...
            {
                {"hostname", Server.ServerConfiguration.MessageOfTheDay},
                {"gametype", "SMP"},
                {"game_id", "Welt_" + Server.ServerConfiguration.Name},
                {"version", "0.4"},
                {"plugins", ""},
                {"map", Server.Worlds.First().Name},
                {"numplayers", Server.Clients.Count.ToString()},
                {"maxplayers", "1024"},
                {"hostport", Server.ServerConfiguration.Port.ToString()},
                {"hostip", Server.ServerConfiguration.Address}
            };
            return stats;
        }
        private IEnumerable<string> GetPlayers()
        {
            return Server.Clients.Select(c => c.Username);
        }
