...
namespace Welt.Core.Forge.Generators
{
    internal class DualLayerTerrainWithMediumValleysForRivers : SimpleTerrain
    {
        ...
        protected sealed override void GenerateTerrain(IWorld world, IChunk chunk, byte blockXInChunk, byte blockZInChunk, uint worldX,
            uint worldZ)
        {
            _mLowerGroundHeight = GetLowerGroundHeight(worldX, worldZ);
            _mUpperGroundHeight = GetUpperGroundHeight(worldX, worldZ, _mLowerGroundHeight);
        ...
        private void GenerateWaterSandLayer(IChunk chunk)
        {
            //BlockType blockType;
            //bool sunlit = true;
                ...
                for (byte z = 0; z < Chunk.Size.Z; z++)
                {
                    ...
                    for (byte y = Waterlevel + 11; y >= Waterlevel; y--)
                    {
                        if ((chunk.GetBlock(x, y, z).Id == BlockType.DIRT) ||
                            (chunk.GetBlock(x, y, z).Id == BlockType.GRASS) ||
                            (chunk.GetBlock(x, y, z).Id == BlockType.LAVA))
                        {
        ...
        private void GenerateTreesFlowers(IChunk chunk)
        {
            for (byte x = 0; x < Chunk.Size.X; x++)
            {
                for (byte z = 0; z < Chunk.Size.Z; z++)
                {
                    ...
                    {
                        if (chunk.GetBlock(x, (byte)y, z).Id == BlockType.GRASS)
                        {
                            if (R.Next(700) == 1)
                            {
