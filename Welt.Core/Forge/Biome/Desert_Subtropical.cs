...
{
    internal class DesertSubtropical : SimpleTerrain
    {
        public override void Generate(IWorld world, IChunk chunk)
        {
        ...
        protected sealed override void GenerateTerrain(IWorld world, IChunk chunk, byte blockXInChunk, byte blockZInChunk, uint worldX,
            uint worldZ)
        {
            var lowerGroundHeight = GetLowerGroundHeight(worldX, worldZ);
            var upperGroundHeight = GetUpperGroundHeight(worldX, worldZ, lowerGroundHeight);

            //bool sunlit = true;

            for (var y = Chunk.Max.Y; y >= 0; y--)
            {
                var blockType = y > upperGroundHeight ? BlockType.NONE : BlockType.SAND;
                ...
                //    sunlit = true;
                //}
                chunk.SetBlock(blockXInChunk, y, blockZInChunk, new Block(blockType));
            }
        }
        ...
        private void GenerateWaterSandLayer(Chunk chunk)
        {
            ...
            for (byte x = 0; x < Chunk.Size.X; x++)
            {
                for (byte z = 0; z < Chunk.Size.Z; z++)
                {
                    ...
                    {
                        //if ((y > 11) && (chunk.Blocks[x, y, z].Id == BlockType.Grass)) chunk.SetBlock(x, y, z, new Block(BlockType.Sand, sunlit));
        ...
        private static int GetUpperGroundHeight(uint blockX, uint blockY, float lowerGroundHeight)
        {
        ...
        private static float GetLowerGroundHeight(uint blockX, uint blockY)
        {
