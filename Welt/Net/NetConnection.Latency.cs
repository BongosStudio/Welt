namespace Welt.Net
{
	public partial class NetConnection
	{
		...
		{
			m_remoteTimeOffset = (remoteSendTime + (m_averageRoundtripTime / 2.0)) - Platform.NetTime.Now;
		}
		...
		{
			double now = Platform.NetTime.Now;
		...
		internal void SendPing()
		{
			...
			m_sentPingTime = Platform.NetTime.Now;
		...
		internal void SendPong(int pingNumber)
		{
			...
			om.Write((float)Platform.NetTime.Now); // we should update this value to reflect the exact point in time the packet is SENT
