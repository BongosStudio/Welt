	...
	internal sealed class NetReliableSenderChannel : NetSenderChannelBase
	{
		...
		private void DestoreMessage(double now, int storeIndex, out bool resetTimeout)
		{
			...
			if (storedMessage == null)
				throw new NetException("m_storedMessages[" + storeIndex + "].Message is null; sent " + m_storedMessages[storeIndex].NumSent + " times, last time " + (Platform.NetTime.Now - m_storedMessages[storeIndex].LastSent) + " seconds ago");
#else
			if (storedMessage != null)
			{
#endif
