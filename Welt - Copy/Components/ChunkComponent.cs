...
namespace Welt.Components
{
    public class ChunkComponent : IVisualComponent
    {
        ...
        private void DrawChunks()
        {
            ...
            BlockEffect.Parameters["HeldItemLight"].SetValue(BlockLogic.GetLightLevel(PlayerRenderer.Player.SelectedItem).ToVector3());
            BlockEffect.Parameters["HasFlicker"].SetValue(PlayerRenderer.Player.SelectedItem.Block.Id == BlockType.TORCH);
            BlockEffect.Parameters["SunColor"].SetValue(World.World.SunColor);
            BlockEffect.Parameters["TimeOfDay"].SetValue((float)tod);
            BlockEffect.Parameters["SkyLightPosition"].SetValue(World.World.SunPos);
            ...
            //Graphics.RasterizerState.CullMode = CullMode.CullClockwiseFace;

            var rendered = 0;
            for (var i = 0; i < m_Meshes.Count; ++i)
            {
                var chunk = m_Meshes[i];
                // we need to figure out why the chunk bounding box doesn't update to it's actual value.
